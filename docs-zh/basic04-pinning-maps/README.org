# -*- fill-column: 76; -*-
#+TITLE: 教程：Basic04 - 映射固定
#+OPTIONS: ^:nil

在本课中，你将学习如何从另一个"外部"程序读取 BPF 映射。

在 basic03 中，[[file:../basic03-map-counter/xdp_load_and_stats.c][xdp_load_and_stats.c]] 程序既加载 BPF 程序又从映射中
读取统计数据。这很实用，因为映射文件描述符可以随时获得；然而，映射
只能从加载它的同一程序访问这一点通常是有限制的。

在本课中，我们将把程序分成两个单独的程序：
 - 一个专注于 BPF/XDP 加载（[[file:xdp_loader.c]]）
 - 一个专注于读取和打印统计数据（[[file:xdp_stats.c]]）
...
- [[#作业][作业]]
  - [[#作业-1-xdp_statsc-重新加载映射文件描述符][作业 1：(xdp_stats.c) 重新加载映射文件描述符]]
  - [[#作业-2-xdp_loaderc-重用固定的映射][作业 2：(xdp_loader.c) 重用固定的映射]]

* basic03 作业的解决方案

[[file:../basic03-map-counter][basic03]] 中的作业已在此 basic04 课程中"解决"或实现。因此，这可以作为
basic03 的参考解决方案。
...
在将 [[file:../basic03-map-counter/xdp_load_and_stats.c][xdp_load_and_stats.c]] 程序分成 [[file:xdp_loader.c]] 和 [[file:xdp_stats.c]] 时，
...
[[https://github.com/libbpf/libbpf/blob/master/src/bpf.h][libbpf github 仓库中的源 bpf.h]]）。

这里的重点是 libbpf 将低级 bpf 系统调用包装器保存在单独的文件 [[https://github.com/libbpf/libbpf/blob/master/src/bpf.h][bpf.h]] 和
[[https://github.com/libbpf/libbpf/blob/master/src/bpf.c][bpf.c]] 中。因此，我们可以通过不与 libbpf.a 链接来缩小二进制文件的大小。
...
和我们的 [[file:../testenv][testenv]] 都会自动将其挂载到 =/sys/fs/bpf/= 下的默认位置。
如果没有，使用上面的命令挂载它。
...
但是，如果你打开 [[file:xdp_loader.c]] 并查看我们的函数 =pin_maps_in_bpf_object()=，
...
（参见 [[#assignment1-xdp_statsc-reload-map-file-descriptor][作业 1]] 了解解决方法）
...
（提示：参见 [[#assignment2-xdp_loaderc-reuse-pinned-map][作业 2]]）
...
*作业* 是在 [[file:xdp_loader.c][xdp_loader]] 中检查是否已经有固定版本的映射 "xdp_stats_map"，
