...
[[https://github.com/libbpf/libbpf/blob/master/src/bpf.h][bpf/bpf.h]] 中）。该 API 还提供"[[https://libbpf.readthedocs.io/en/latest/libbpf_naming_convention.html#objects][对象]]"以及处理它们的函数
（定义在 [[https://github.com/libbpf/libbpf/blob/master/src/libbpf.h][bpf/libbpf.h]] 中）。
...
套接字的对象和函数，定义在 [[https://github.com/xdp-project/xdp-tools/blob/master/headers/xdp/libxdp.h][xdp/libxdp.h]] 中。
...
在 [[file:xdp_loader.c]] 中，函数 =xdp_program__create()= 用于创建
...
设置。[[file:xdp_loader.c][加载器代码]] 还需要使用更高级的 libbpf API 调用
=bpf_prog_load_xattr()=，它允许我们设置 ifindex，因为这在加载时需要以
启用硬件卸载。

关于如何在 Netronome 的 Agilio SmartNIC 上使硬件卸载工作的一些详细信息
在 [[file:xdp_offload_nfp.org]] 中；例如，固件需要更新以支持 eBPF。
...
建立一个测试实验室环境。在 [[file:../testenv/][testenv/]] 目录中，你会找到一个脚本
...
在 [[file:xdp_prog_kern.c]] 中添加一个新的程序节 "xdp_abort"，它使用（返回）
