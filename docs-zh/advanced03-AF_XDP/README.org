# -*- fill-column: 76; -*-
#+TITLE: 高级：通过 AF_XDP 实现 XDP 用户空间传递
#+OPTIONS: ^:nil
...
（[[https://www.kernel.org/doc/html/latest/networking/af_xdp.html][AF_XDP 的内核文档]]）。
...
AF_XDP 背后的基本思想之一可以追溯到 [[https://en.wikipedia.org/wiki/Van_Jacobson][Van Jacobson]] 关于
[[https://lwn.net/Articles/169961/][网络通道]] 的演讲。它是关于创建一个从驱动程序 RX 队列直接到
（AF_XDP）套接字的无锁 [[https://lwn.net/Articles/169961/][通道]]。
...
Van Jacobson 谈到的 [[http://www.lemis.com/grog/Documentation/vj/lca06vj.pdf][传输签名]] 被 XDP/eBPF 程序选择 XDP_REDIRECT
到哪个 AF_XDP 套接字所取代。

** 详情：实际上有四个 SPSC 环队列

如 [[https://www.kernel.org/doc/html/latest/networking/af_xdp.html][AF_XDP 内核文档]] 中所解释的，实际上有 4 个 SPSC 环队列。
...
否则将加载 libxdp 的 [[https://github.com/xdp-project/xdp-tools/blob/master/lib/libxdp/xsk_def_xdp_prog.c][默认 AF_XDP]] 程序。
