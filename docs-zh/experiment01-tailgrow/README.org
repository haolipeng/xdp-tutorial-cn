...
在主示例 [[file:xdp_prog_kern.c]] 中，=xdp_tailgrow_parse= 代码通过解析到
IP 层，并使用 IP 头部的总长度字段（[[https://elixir.bootlin.com/linux/v5.6.10/source/include/uapi/linux/ip.h#L97][iphdr->tot_len]]）来实现这一点。
查看代码了解说服验证器所需的奇怪边界检查。请注意，为测试目的，
这仅限于 IPv4 ICMP 数据包。

** 附注：额外程序

附注：[[file:xdp_prog_kern.c]] 还包含一些其他较小的程序来测试
...
[[file:xdp_prog_kern2.c]] 中的代码展示了如何找到 =data_end=，
*不解析数据包内容*，而是通过在有界循环中一次将 =data= 位置指针
向前移动一个字节。具有最大迭代次数的有界循环允许验证器看到边界。
（这显然依赖于在内核 [[https://git.kernel.org/torvalds/c/v5.3-rc1~140^2~179^2^2~5][v5.3]] 中添加的有界循环支持）。
这不是很高效，但它有效。
...
在示例 [[file:xdp_prog_fail1.c]] 中，我们尝试使用数据包长度
...
在示例 [[file:xdp_prog_fail2.c]] 中，我们尝试或多或少直接使用 =data_end=
指针来找到数据包中的最后一个字节。数据包数据 [[https://www.mathwords.com/i/interval_notation.htm][区间]] 定义为
