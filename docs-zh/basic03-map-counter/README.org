...
创建 BPF 映射是通过在 [[file:xdp_prog_kern.c]] 中定义一个全局结构体，
使用特殊的 =SEC(".maps")= 如下：
...
正如你在 [[file:../basic02-prog-by-name/][basic02]] 中学到的，libbpf API 有"对象"和处理这些对象的函数。
结构体 =bpf_object= 代表 ELF 对象本身。

与我们对 BPF 函数所做的类似，我们的加载器有一个名为 =find_map_fd()=
的函数（在 [[file:xdp_load_and_stats.c]] 中），它使用库函数
...
BPF 映射不了解用于值记录的数据结构，它只知道大小。（[[https://github.com/torvalds/linux/blob/master/Documentation/bpf/btf.rst][BPF 类型格式]]（[[https://www.kernel.org/doc/html/latest/bpf/btf.html][BTF]]）
...
这个 =struct datarec= 在包含文件 [[file:common_kern_user.h]] 中定义为：
...
下一步是更新内核端 BPF 程序：[[file:xdp_prog_kern.c]]。

要弄清数据包的长度，你需要了解 BPF 程序在被内核调用时获得指针的上下文
变量 =*ctx=，类型为 [[https://elixir.bootlin.com/linux/v5.0/ident/xdp_md][struct xdp_md]]。这个 =struct xdp_md= 有点奇怪，
...
现在是时候更新读取统计数据的用户空间程序了（在 [[file:xdp_load_and_stats.c]] 中）。
...
=max_entries=XDP_ACTION_MAX=。这个想法是按 [[https://elixir.bootlin.com/linux/latest/ident/xdp_action][(enum) xdp_action]] 保存统计数据，
但我们的程序还没有利用这一点。

*作业* 是扩展用户空间统计工具（在 [[file:xdp_load_and_stats.c]] 中）
以收集和打印这些额外的统计数据。
